...
def cleanup_attachments() -> None:
    ...
    for att in deleted:
        if att.local_path and os.path.exists(att.local_path):
            try:
                ...
                logger.info({"event": "attachment_deleted", "path": att.local_path})
            except OSError as e:
                logger.warning({"event": "attachment_delete_error", "path": att.local_path, "error": str(e)})
    ...
    for filename in os.listdir(attach_dir):
        ...
        if full_path not in active_paths:
            try:
                os.remove(full_path)
                logger.info({"event": "orphan_attachment_removed", "path": full_path})
            except OSError as e:
                logger.warning({"event": "orphan_remove_error", "path": full_path, "error": str(e)})
